        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if (isset($responseData['data']['id'])) {
                    $diseaseId = $responseData['data']['id'];
                    ...
                    $responseCrops = Http::post($url . '/v1/diseases/' . $diseaseId . '/crops', [
                        'crop_ids' => $cropIds,
                    ]);

                    error_log('URL de la solicitud: ' . $url . '/v1/diseases/' . $diseaseId . '/crops');
                    $responseCrops = Http::post($url . '/v1/diseases/' . $diseaseId . '/crops', ['crop_ids' => $cropIds]);
                    ...
                    // info('Trying to associate crops to disease: ' . $url . '/v1/diseases/' . $diseaseId . '/crops');
                    ...
                    if ($responseCrops->successful()) {
                        ...
                        $errorMessage = $responseCrops->json()['message'] ?? 'Ups.. Error al asociar cultivos a la enfermedad';
                        $message = 'Ups.. la enfermedad se creó pero no se pudieron asociar los cultivos: ' . $errorMessage;
                        return redirect()->route('diseases.index')->with('error', $message);
                    }
                }
            }
            ...
            $message = 'Ups.. la enfermedad se creó pero no se pudo obtener el ID';
            return redirect()->route('diseases.index')->with('error', $message);
        } catch (\Throwable $e) {
